#!/usr/bin/env python3
"""
Concurrently run multiple MCMC chains of PhyloWGS.

Python 3 port: queue → queue.Queue, scipy.misc.logsumexp → scipy.special.logsumexp.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        ...
        'All options that evolve.py accepts may also be specified here.',
    ...
    parser.add_argument('-I', '--chain-inclusion-factor', dest='chain_inclusion_factor',
...
def check_args(args):
    ...
    if args['random_seeds'] is not None and len(args['random_seeds']) != args['num_chains']:
        raise ValueError(
            "Must specify exactly one seed for each of %s chain(s). You specified %s seed(s)."
...
def run_chains(args, other_args):
    ...
    for chain_index in range(args['num_chains']):
        ...
        process = run_chain(
            chain_index, args['ssm_file'], args['cnv_file'],
            app_dir, working_dir, output_dir, args['random_seeds'], other_args,
        )
...
def watch_chains(processes):
    ...
    while True:
        if set(S['status'] for S in status.values()) == {'done'}:
            break
        ...
        for chain_index in sorted(chain_stdout.keys()):
            for line in chain_stdout[chain_index]:
                ...
                if line.startswith('iteration='):
                    ...
                    status[chain_index]['percent_complete'] = '{:.2f}%'.format(
                        100 * float(status[chain_index]['trees_sampled'])
                        / float(status[chain_index]['total_trees'])
                    )
                else:
                    if len(line) > 0:
        ...
        if last_lines_were_status and sys.stdout.isatty():
            print("\033[2K\033[1A" * (num_chains + 1))
        for cidx in sorted(status.keys()):
            if status[cidx]['status'] == 'running':
                ...
                keys = tuple()
            status_msg = ' '.join(
                '{}={}'.format(K, status[cidx][K]) for K in ('status',) + keys
            )
...
def merge_best_chains(out_dir, chain_dirs, included_chains, excluded_chains):
    ...
    combined_tree_zipfile = zipfile.ZipFile(
        combined_fn, mode='w', compression=zipfile.ZIP_DEFLATED, allowZip64=True
    )
    logmsg("Including chains {}".format(
        ' '.join('{}={}'.format(cidx, logsumlh) for cidx, logsumlh in included_chains)
    ))
    if excluded_chains:
        logmsg("Excluding chains {}".format(
            ' '.join('{}={}'.format(cidx, logsumlh) for cidx, logsumlh in excluded_chains)
        ))
    else:
    ...
    write_results_path = os.path.normpath(
        os.path.join(os.path.dirname(__file__), 'write_results.py')
    )
    logmsg("Chain merging complete.")
    logmsg("You may remove the following unneeded intermediate files: {}".format(
        ' '.join(zip_paths)
    ))
    logmsg('To write JSON results, please run `{} {} {} {} {} {} {}`'.format(
        sys.executable, write_results_path,
        'run_name', combined_fn,
        'run_name.summ.json.gz', 'run_name.muts.json.gz', 'run_name.mutass.zip',
    ))
...
def main():
    ...
    included_chains, excluded_chains = determine_chains_to_merge(
        chain_dirs, args['chain_inclusion_factor']
    )
